...
class SoftReIdentifierTest(unittest.TestCase):
    ...
    def test_reacquires_recent_matching_identity(self):
        ...
        reid.prepare_frame(set(), now=3)
        reid.prepare_frame(set(), now=5)
    ...
    def test_does_not_reacquire_after_ttl(self):
        reid = SoftReIdentifier(ttl_seconds=10, similarity_threshold=0.8)
        first = reid.assign(1, [1.0, 0.0], now=1)
        reid.prepare_frame(set(), now=2)
        reid.prepare_frame(set(), now=4)
        second = reid.assign(2, [1.0, 0.0], now=13)
        self.assertNotEqual(first[0], second[0])

    def test_single_missed_frame_keeps_track_mapping(self):
        reid = SoftReIdentifier(track_grace_seconds=1.5)
        first = reid.assign(3, [1.0, 0.0], now=1)
        reid.prepare_frame(set(), now=1.1)
        reid.prepare_frame({3}, now=1.2)
        second = reid.assign(3, None, now=1.2)
        self.assertEqual(first[0], second[0])
